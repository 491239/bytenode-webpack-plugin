...
async function compileSource(source: Source, options: Pick<Options, 'compileAsModule' | 'compileForElectron' | 'electronPath'>): Promise<Source> {
  return await replaceSource(source, async raw => {
    ...
    return options.compileForElectron
      ? await compileElectronCode(raw, options.electronPath ? { electronPath: options.electronPath } : undefined)
      : compileCode(raw, false);
  });
}
